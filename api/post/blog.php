...
if ($uid && $sessionUserId == $uid ) {
    ...
    if($type) {
        ...
        if(isset($infoUpdate["id"]) && $infoUpdate["id"] ) {
            ...
            if ($db->db_update($infoUpdate, TABLE_BLOG, array("id" => $iId, "ui" => $uid))) {
            ...
            if ($db->db_insert($infoUpdate, TABLE_BLOG)) {
                $str_query = "SELECT * FROM ".TABLE_BLOG." WHERE cr={$currentTime} AND ui={$uid} LIMIT 0,1";
                $row = $db->db_array($str_query);

                if ($row) {
                    $file = FOLDERBLOG . "{$row["id"]}.xml";
                    $information = array(
                        "db" => $row,
                    );
                    saveXMLFile($file, $information);
        ...
        $iId = isset($post["db"]["id"]) ? $post["db"]["id"]:null;
        if($iId) {
            if($db->db_delete(TABLE_BLOG, array("id"=>$iId)) ){
                # delete image, xml, slide file
                $fileXml = FOLDERBLOG . "{$iId}.xml";
                $fileImage = "";
                if (is_file($fileXml)) {
                    // delete image file
