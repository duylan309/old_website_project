...
if(isset($_GET["accessToken"]) && $_GET["accessToken"]) {
	...
	if($_SERVER['HTTP_HOST'] == $strLinkReferer && isset($_SESSION["createAccessTokenFB"]) && $_SESSION["createAccessTokenFB"] == $accessToken ) {
		...
		$education         = isset($data_get_json["education"]) ? $data_get_json["education"] : null;
		$work              = isset($data_get_json["work"]) ? $data_get_json["work"] : null;
		...
		$row = null;
		if($post["email"] != null && $post["email"]!= "" && count($post["email"])){
			
			$str_query  = " SELECT id,fbid, email, username, name, im, gender, dob, deactive, type, status, created, DAY(dob) AS day, MONTH(dob) AS month, YEAR(dob) AS year
							FROM ".TABLE_USER." 
			...
			$str_query  = " SELECT id,fbid, email, username, name, im, gender, dob, deactive, type, status, created, DAY(dob) AS day, MONTH(dob) AS month, YEAR(dob) AS year
							FROM ".TABLE_USER." 
							WHERE fbid='$uid' AND type=2 AND signupby='FB' LIMIT 0,1";
			# echo $str_query;
		}	
		...
		if ($row ) {
			...
			if($row["fbid"] == ""){
				$strUpdateLastLogin = " UPDATE user 
									SET last_signin=$currentTime,fbid=$uid
									WHERE id = $userId";
			}else{
				$strUpdateLastLogin = " UPDATE user 
			...
			if ($db->db_insert($post, TABLE_USER)) {
				...
				$str_query = "  SELECT id, email, username, name, im, gender, dob, deactive, type, status, created, DAY(dob) AS day, MONTH(dob) AS month, YEAR(dob) AS year, last_signin
								FROM user
