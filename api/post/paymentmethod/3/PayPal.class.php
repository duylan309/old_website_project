...
class PayPal {
	...
	public function __construct($env = 'LIVE', $username, $password, $signature) {
		if ($env != 'LIVE' && $env != 'SANDBOX') {
			$this->error('INVALID_ENVIRONMENT');
		}
	...
	public function setExpressCheckout($returnURL, $cancelURL) {
		if (!isset($_GET['token'])) {
			...
			if ($this->execute()) {
				$urlSubmit = $this->paypalURLs[$this->environment] . 'webscr?cmd=_express-checkout&token=' . $this->response['TOKEN'];
	...
	public function reverseTransaction($transaction) {
		if (!ctype_alnum($transaction) || strlen($transaction) != 17) {
			$this->error('INVALID_TRANSACTIONID');
		}
	...
	public function setCardDetails($type, $number, $expiry, $cvv2) {
		...
		if (!array_key_exists($type, $this->cardTypes)) {
			$this->error('INVALID_CARD_TYPE');
		} elseif (!is_numeric($number)) {
			$this->error('INVALID_CARD_NUMBER');
		} elseif (!$this->cardValidate($type, $number)) {
			$this->error('CARD_VALIDATION_FAILED');
		} elseif (!is_int($expiry)) {
			$this->error('INVALID_EXPIRY_DATE');
		} elseif (!is_int($cvv2)) {
			$this->error('INVALID_CARD_CVV2');
		} else {
	...
	private function cardValidate($type, $number) {
		if (preg_match($this->cardTypes[$type], $number)) {
			...
			if (array_sum(str_split($number)) % 10 == 0) {
				return true;
			}
	...
	public function addRecipient($email, $amt) {
		...
		if (!is_numeric($amt)) {
			$this->error('INVALID_PRICE');
		} elseif (count($this->recipients) >= 250) {
			$this->error('MAX_RECIPIENTS');
		} else {
	...
	public function setCurrencyCode($currency) {
		if (!in_array($currency, $this->currencies)) {
			$this->error('INVALID_CURRENCY_CODE');
		} else {
