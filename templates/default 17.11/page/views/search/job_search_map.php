        ...
        <div class="col-sm-3">
            <div class="filter-list filter-map search-job hidden-xs transition">
                <form class="post-form filter-form" action="/<?=$seo_name["page"]["search"]?>">
                    ...
                    <div class="btn btn-block bg-color5 m-t-10 hidden" id="nearby">
                        <i class="fa fa-location-arrow"></i> <span><?=$language["jobNearMe"]?></span>
                    </div>
                    

                    <?php $link_search = str_replace(array("&map=view","?map=view"),"","http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]")?>    
                    <a class="btn btn-block bg-color2 m-t-10" href="<?=$link_search?>">
                        <i class="fa fa-th"></i> <span><?=$language["viewByList"]?></span>
                    </a>
                    <a class="btn btn-block bg-color7" href="<?=SITEURL.$seo_name["page"]["search"].'?random=1&brand=view'?>">
