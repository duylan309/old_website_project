<div class="col-sm-9">
	<div class="message-detail">
		<div class="row">
			...
			<div class="col-sm-3 text-right">
				<button class="btn bg-color7"
						...
						data-params='{  "db":{  "user_id"     : "<?=$sessionUserId?>",
		...
		<div class="content cmp-more b-r-4 b-b">
			...
			<div class="load-old-messages">
				<div class="text-color2"><?=$language["loadoldmessage"]?></div>
			</div>
			<?php }?>
			<?php if($old_message["receiver_id"] != $sessionUserId){?>
			<div data-old-message <?=count($old_message_array) > 1 && $i==1 ? "data-overflow" :""?> <?=$i == count($old_message_array) || $i == 1 ? "data-show" : ""?> >
				<div data-header class="row">
					...
					<div class="col-sm-9">
						...
						<p><span class="text-color4"><?=$language["from"]?>:</span> <?=$old_message["user_name"]?></p>
					</div>
					<div class="col-sm-2 text-right">
						<?=strDate($old_message["created_date"])?>
				...
				<div data-content class="row b-b m-b-15">
					<div class="col-sm-offset-1 col-sm-11">
						<div class="textarea-content-line"><?=trim($old_message['message'])?></div>
					</div>
				</div>
			</div>
			<?php }else{?>
			<div data-old-message <?=count($old_message_array) > 1 && $i==1 ? "data-overflow" :""?> <?=$i == count($old_message_array) || $i == 1 ? "data-show" : ""?> >
				<div data-header class="row">
					...
					<div class="col-sm-9">
						...
						<p><span class="text-color4"><?=$language["from"]?>:</span> <?=$old_message["company_name"]?></p>
					</div>
					<div class="col-sm-2 text-right">
						<?=strDate($old_message["created_date"])?>
						<div data-message-item>
			...
			<div data-header class="row">
				...
				<div class="col-sm-9">
					...
					<p><span class="text-color4"><?=$language["from"]?>:</span> <?=$message_detail["company_name"]?></p>
				</div>
				<div class="col-sm-2 text-right">
					<?=strDate($message_detail["created_date"])?>
					<div data-message-item>
							<div class="btn-favorite show-unfavorite-<?=$old_message["id"]?> <?=$old_message["important"] == 1 ? "active" : ""?>">
							    <span class="unfavorite text-color2"
							            data-button-magic
							            ...
							            data-format-json="true"
							            data-ajax-url="<?=APIPOSTMESSAGES?>"
							            data-success-toggle-class=".show-unfavorite-<?=$old_message[0]["id"]?>,active"
							            data-params='{  "db":{  "employer_id" : "<?=$old_message[0]["employer_id"]?>",
							                                    "company_id"  : "<?=$old_message[0]["company_id"]?>",  
							                                    "user_id"     : "<?=$old_message[0]["user_id"]?>",
							                                    "sender_id"   : "<?=$old_message[0]["sender_id"]?>",
							                                    "receiver_id" : "<?=$old_message[0]["receiver_id"]?>",
							                                    "message_id"  : "<?=$old_message[0]["id"]?>" },
							                            "action" : "unimportant" 
							                        }'
							            data-show-success=".alert-footer.alert"
							            data-show-errors=".alert-footer.alert-error"
			...
			<div data-content class="row b-b">
				<div class="col-sm-offset-1 col-sm-11">
					<div class="textarea-content-line"><?=trim($message_detail['message'])?></div>
				</div>
			</div>
			<?php }?>
			<div data-reply class="row m-t-15">
				<div class="col-sm-1">
					<?php if($_SESSION["userlog"]["im"] != null && $_SESSION["userlog"]["im"] != ""):?>
					<img alt="<?=$_SESSION["userlog"]["name"]?>"
					class="img-responsive b-r-4"
					src="/<?=FOLDERIMAGEUSER?>thumbnail/<?=$_SESSION["userlog"]["im"]?>">
					<?php else:?>
					<img alt="<?=$_SESSION["userlog"]["name"]?>"
					class="img-responsive b-r-4"
				...
				<div class="col-sm-11">
					<form data-submit-message method="post" class="form-horizontal post-form">
						...
						<div class="hidden">
							<input data-user-id name="db.user_id" type="text" value="<?=isset($_SESSION["userlog"]["id"]) ? intval($_SESSION["userlog"]["id"]) : "" ?>">
							...
							<input name="db.receiver_id" type="text" value="<?=$message_detail["employer_id"]?>">
							<input name="db.employer_id" type="text" value="<?=$message_detail["employer_id"]?>">
