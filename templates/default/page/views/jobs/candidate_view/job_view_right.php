...
if($sessionUserId != $infoCmp["ui"]) {
    $strQueryJobCmp = " SELECT job.id
                        FROM ".TABLE_JOB." AS job, ".TABLE_USER." AS user
                        WHERE job.ui = user.id 
                        AND user.deactive = 0 
                        AND user.dayleft > {$currentTime}
                        AND job.ci = {$infoCmp["id"]} 
                        AND job.id <> {$jid} 
                        AND job.st=2 
                        LIMIT 0,3 ";
 ...
 if($title) {
     foreach ($title as $key => $value) {
         if($value) {
             ...
             $strWhere .= $key == 0 ? " job.ti LIKE '%{$value}%' " : " OR job.ti LIKE '%{$value}%' ";
         }
     }
 }

 $strQueryJobOther = " SELECT job.id
                        FROM ".TABLE_JOB." AS job, ".TABLE_USER." AS user
                        WHERE job.ui = user.id 
                        AND user.deactive = 0 
                        AND user.dayleft > {$currentTime}
