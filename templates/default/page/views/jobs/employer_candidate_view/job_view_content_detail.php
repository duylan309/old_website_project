...
<div class="cmp-more"> -->
    ...
    <?php if(isset($infoJob["more"]["workhour"]) && count($infoJob["more"]["workhour"])):?>
    <div class="j-view-option row">
        <div class="col-sm-3 col-xs-4"> <label> </label></div>
        <div class="col-sm-9 col-xs-8"> 
          <p class="textarea-content"><?=isset($infoJob["more"]["workhour"]) && count($infoJob["more"]["workhour"]) ? preg_replace('/(?<!\d)[.,!?](?![.,!?\d])/', ', ', $infoJob["more"]["workhour"]) : "" ?></p>
        </div>
    </div>
    <?php endif;?>
