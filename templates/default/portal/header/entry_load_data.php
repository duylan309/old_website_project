...
if($sessionUserId):
    if($_SESSION["userlog"]["type"] == 1) {
        ...
        $strSelectNoti = "CONCAT ( u.name , ' ', j.ti) AS na,
                         u.id AS ui,
                         u.name AS na,
                         u.im AS im,
                         ja.jo AS ji,
        ...
        if($_SESSION["userlog"] && isset($_SESSION["usersub"])){
            $strWhereNoti  = "WHERE u.id = ja.ui AND ja.ei={$_SESSION["userlog"]["id"]} AND j.id=ja.jo AND j.ci={$_SESSION["usersub"]["id"]} ";
        }else{
            $strWhereNoti  = "WHERE u.id = ja.ui AND ja.ei={$_SESSION["userlog"]["id"]} AND j.id=ja.jo ";
        }
        ...
        $strWhere .= "  AND comment.uid    = {$_SESSION["userlog"]["id"]}
                        AND comment.pid    = company.id 
                        AND comment.status = 1";

        # SHOW STATUS
        $strQueryNoti  = "  SELECT $strSelect
                            FROM ".TABLE_COMMENT." AS comment,
                                 ".TABLE_COMPANY." AS company
                            WHERE 1 = 1 {$strWhere} ORDER BY comment.id DESC LIMIT 10";
        $listNotification = $db->db_arrayList($strQueryNoti);
        $strQueryTotal = "SELECT COUNT(id) AS total FROM ".TABLE_COMMENT." WHERE uid = {$_SESSION["userlog"]["id"]} AND readed = 0";
        $totalNotification = $db->db_array($strQueryTotal);
        $totalNotification["total"] = count($totalNotification) == 0 ? 0 : $totalNotification["total"];
    }
